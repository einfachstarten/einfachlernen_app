<?php
// admin/analytics.php - Clean Activity-Based Analytics Dashboard
session_start();
...
// KPI Metrics from customer_activities
function getKPIMetrics($pdo, $days) {
    try {
        ...
        return ['active_users' => 0, 'total_logins' => 0, 'slot_searches' => 0, 'booking_attempts' => 0, 'bookings_completed' => 0, 'today_activities' => 0];
    }
}
...
function getConversionFunnel($pdo, $days) {
    try {
        ...
        return ['users_logged_in' => 0, 'users_viewed_services' => 0, 'users_found_slots' => 0, 'users_started_booking' => 0, 'users_completed_booking' => 0];
    }
}
...
<body>
    <div class="container">
        <!-- Header -->
        <div class="header">
            <h1 class="dashboard-title">📊 Customer Analytics</h1>
        ...
        <div class="kpi-grid">
            <div class="kpi-card">
                <div class="kpi-number" style="color: #667eea;"><?= $kpis['active_users'] ?></div>
                <div class="kpi-label">Active Users</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-number" style="color: #28a745;"><?= $kpis['total_logins'] ?></div>
                <div class="kpi-label">Total Logins</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-number" style="color: #ffc107;"><?= $kpis['slot_searches'] ?></div>
                <div class="kpi-label">Slot Searches</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-number" style="color: #17a2b8;"><?= $kpis['booking_attempts'] ?></div>
                <div class="kpi-label">Booking Attempts</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-number" style="color: #dc3545;"><?= $kpis['bookings_completed'] ?></div>
                <div class="kpi-label">Completed Bookings</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-number" style="color: #6f42c1;"><?= $conversion_rates['booking_completion'] ?>%</div>
                        ...
                        <tbody>
                            <?php foreach (array_slice($top_customers, 0, 5) as $customer): ?>
                                <tr>
                                    ...
                                    <td><strong style="color: #667eea;"><?= $customer['activity_count'] ?></strong></td>
                                    <td style="color: #718096;"><?= date('M j, Y', strtotime($customer['last_activity'])) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
        ...
        if (ctx && trendsData && trendsData.length > 0) {
            ...
            try {
                ...
                document.querySelector('.chart-container').innerHTML = `
                    <div style="padding: 40px; text-align: center; color: #e53e3e; background: #fed7d7; border-radius: 8px;">
                        ⚠️ Chart konnte nicht geladen werden<br>
                        <small>Bitte Seite neu laden</small>
                    </div>
                `;
            }
        } else {
